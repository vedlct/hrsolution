...
//            $leaves=$leaves->where(DB::raw('date(hrmleaves.startDate)'),'=<',$r->startDate)
//                ->where(DB::raw('date(hrmleaves.endDate)'),'=>',$r->endDate);
            $leaves = $leaves->whereRaw("date(hrmleaves.startDate) between '".$r->startDate."' and '".$r->endDate."'  ");
        }
        $leaves=$leaves->get();
